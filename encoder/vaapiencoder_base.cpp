...
void VaapiEncoderBase::fill(VAEncMiscParameterFrameRate* frameRate, uint32_t temporalID) const
{
    ...
    frameRate->framerate = (temporalID == m_videoParamCommon.temporalLayers.length) ? fps() : m_svctFrameRate[temporalID].frameRateNum | (m_svctFrameRate[temporalID].frameRateDenom << 16);
}
